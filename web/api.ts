import { App, DeleteHostQuery, DeleteUserRequest, DetailedHost, DetailedUser, GetAppImageQuery, GetAppsQuery, GetAppsResponse, GetHostQuery, GetHostResponse, GetHostsResponse, GetUserQuery, GetUsersResponse, HostCapabilityProfile, PatchUserRequest, PostCancelRequest, PostCancelResponse, PostLoginRequest, PostPairRequest, PostPairResponse1, PostPairResponse2, PostUserRequest, PostWakeUpRequest, PostHostRequest, PostHostResponse, RefreshHostCapabilityResponse, UndetailedHost, PatchHostRequest } from "./api_bindings.js";
...
export async function getApi(): Promise<Api> {
    ...
    const nativeLaunchBootstrapState = await apiBootstrapNativeLaunchSession(api)
    if (nativeLaunchBootstrapState == "success" && await apiAuthenticate(api)) {
        return api
    }
    if (isWindowsNativeShellPage()) {
        const message = nativeLaunchBootstrapState == "failed"
            ? "Native Windows session token sudah tidak valid. Kembali ke website lalu tekan Open App lagi."
            : "Native Windows session belum siap. Kembali ke website lalu buka ulang sesi ini."
        showErrorPopup(message, true)
...
type NativeLaunchBootstrapState = "not_applicable" | "success" | "failed"

function readCurrentQueryParam(name: string): string | null {
    try {
...
function isWindowsNativeShellPage(): boolean {
    return readCurrentQueryParam("nativeShell") == "windows"
}

function stripConsumedLaunchTokenFromUrl() {
    try {
        const nextUrl = new URL(window.location.href)
        if (!nextUrl.searchParams.has("launchToken")) {
            return
        }
        nextUrl.searchParams.delete("launchToken")
...
async function apiBootstrapNativeLaunchSession(api: Api): Promise<NativeLaunchBootstrapState> {
    const launchToken = readCurrentQueryParam("launchToken")
    if (!launchToken) {
        return "not_applicable"
    }

    try {
        await fetchApi(api, "/android-native/bootstrap-web-session", POST, {
            json: {
                launch_token: launchToken,
            },
            response: "ignore",
        })
        stripConsumedLaunchTokenFromUrl()
        return "success"
    } catch (error) {
        if (error instanceof FetchError) {
...
export async function apiGetHostCapabilityProfile(api: Api): Promise<HostCapabilityProfile> {
    const response = await fetchApi(api, "/admin/host-capability", GET)

    return response as HostCapabilityProfile
}
export async function apiGetHostOperationsStatus(api: Api): Promise<HostOperationsStatus> {
    const response = await fetchApi(api, "/admin/host-ops-status", GET)

    return response as HostOperationsStatus
}
export async function apiRefreshHostCapabilityProfile(api: Api): Promise<HostCapabilityProfile> {
    const response = await fetchApi(api, "/admin/host-capability/refresh", POST)

    return (response as RefreshHostCapabilityResponse).profile
}
