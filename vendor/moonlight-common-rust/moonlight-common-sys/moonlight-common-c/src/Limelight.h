...
// This function attempts to update the active stream bitrate without tearing down the existing
// connection. It re-announces the current stream configuration to the host with the new bitrate
// and then requests an IDR frame so the encoder can converge on the new target.
// Returns 0 on success or a negative / RTSP status error code on failure.
