...
void cleanupRtspHandshakeState(void) {
...
static const char* getDefaultVideoAnnounceTarget(void) {
    return APP_VERSION_AT_LEAST(7, 1, 431) ? controlStreamId : "streamid=video";
}

static bool sendVideoAnnounceForTarget(PRTSP_MESSAGE response, const char* target, int* error) {
    RTSP_MESSAGE request;
    bool ret;
    int payloadLength;
...
static bool sendVideoAnnounce(PRTSP_MESSAGE response, int* error) {
    return sendVideoAnnounceForTarget(response, getDefaultVideoAnnounceTarget(), error);
}

static bool tryLiveVideoPlayRefresh(const char* playTarget) {
    RTSP_MESSAGE response;
    int error = -1;

    if (!playStream(&response, (char*)playTarget, &error)) {
        Limelog("RTSP live PLAY refresh %s failed: %d\n", playTarget, error);
        return false;
    }

    if (response.message.response.statusCode != 200) {
        Limelog("RTSP live PLAY refresh %s failed with status: %d\n",
            playTarget,
...
static int tryLiveVideoAnnounceTarget(const char* announceTarget, const char* playTarget) {
    RTSP_MESSAGE response;
    int error = -1;

    Limelog("RTSP live bitrate update trying ANNOUNCE target: %s\n", announceTarget);

    if (!sendVideoAnnounceForTarget(&response, announceTarget, &error)) {
        Limelog("RTSP live bitrate update ANNOUNCE target %s transact failed: %d\n",
            announceTarget,
            error);
        return error;
    }
    ...
    if (response.message.response.statusCode != 200) {
        int statusCode = response.message.response.statusCode;
        Limelog("RTSP live bitrate update ANNOUNCE target %s rejected with status: %d\n",
            announceTarget,
            statusCode);
        freeMessage(&response);
        return statusCode;
    ...
    if (playTarget != NULL) {
        tryLiveVideoPlayRefresh(playTarget);
    }
...
int LiUpdateStreamBitrate(int bitrate) {
    int previousBitrate;
    int lastError = -1;
    const char* announceTargets[8];
    int announceTargetCount;
    int i;
    ...
    if (!hasSessionId || sessionIdString == NULL) {
        return -1;
    }

    previousBitrate = StreamConfig.bitrate;
    ...
    announceTargetCount = 0;

    announceTargets[announceTargetCount++] = getDefaultVideoAnnounceTarget();

    {
        const char* genericControlTarget = "streamid=control";
        bool duplicate = false;
        for (i = 0; i < announceTargetCount; i++) {
            if (strcmp(announceTargets[i], genericControlTarget) == 0) {
                duplicate = true;
                break;
            }
        }
        if (!duplicate) {
            announceTargets[announceTargetCount++] = genericControlTarget;
        }
    }

    {
        const char* setupControlTarget = APP_VERSION_AT_LEAST(7, 1, 431) ? "stream=control/13/0" : "stream=control/1/0";
        bool duplicate = false;
        for (i = 0; i < announceTargetCount; i++) {
            if (strcmp(announceTargets[i], setupControlTarget) == 0) {
                duplicate = true;
                break;
            }
        }
        if (!duplicate) {
            announceTargets[announceTargetCount++] = setupControlTarget;
        }
    }

    if (AppVersionQuad[0] >= 5) {
        const char* setupVideoTarget = "streamid=video/0/0";
        bool duplicate = false;
        for (i = 0; i < announceTargetCount; i++) {
            if (strcmp(announceTargets[i], setupVideoTarget) == 0) {
                duplicate = true;
                break;
            }
        }
        if (!duplicate) {
            announceTargets[announceTargetCount++] = setupVideoTarget;
        }
    }

    {
        const char* legacyVideoTarget = "streamid=video";
        bool duplicate = false;
        for (i = 0; i < announceTargetCount; i++) {
            if (strcmp(announceTargets[i], legacyVideoTarget) == 0) {
                duplicate = true;
                break;
            }
        }
        if (!duplicate) {
            announceTargets[announceTargetCount++] = legacyVideoTarget;
        }
    }

    {
        bool duplicate = false;
        for (i = 0; i < announceTargetCount; i++) {
            if (strcmp(announceTargets[i], rtspTargetUrl) == 0) {
                duplicate = true;
                break;
            }
        }
        if (!duplicate) {
            announceTargets[announceTargetCount++] = rtspTargetUrl;
        }
    }

    for (i = 0; i < announceTargetCount; i++) {
        const char* announceTarget = announceTargets[i];
        const char* playTarget = APP_VERSION_AT_LEAST(7, 1, 431) ? "/" : NULL;
        int result = tryLiveVideoAnnounceTarget(announceTarget, playTarget);
        if (result == 0) {
            return 0;
        }
        lastError = result;
    ...
    StreamConfig.bitrate = previousBitrate;
    return lastError;
}
