    ...
    pub async fn start_stream(
        ...
        if current_game != 0 && !has_valid_rtsp_session_url(rtsp_session_url.as_deref()) {
            ...
            for attempt in 0..3 {
                ...
                sleep(Duration::from_millis(280 * (attempt + 1) as u64)).await;
                self.update().await?;
                if self.current_game().await? == 0 {
                    break;
                }
            }
            ...
            rtsp_session_url = loop {
                ...
                let launch_result = {
                    let client = self.client.lock().await;
                    client
                        .send_https::<LaunchEndpoint>(launch_client_info, &https_address, &request)
                        .await
                };
                ...
                match launch_result {
                    ...
                    Err(error) => {
                        ...
                        if launch_attempt < 2 && error_text.contains("concurrent stream") {
                            ...
                            sleep(Duration::from_millis(420 * (launch_attempt + 1) as u64)).await;
                            self.update().await?;
                            launch_attempt += 1;
                            continue;
                        }
