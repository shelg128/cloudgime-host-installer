...
pub struct Config {
    ...
    #[serde(default = "default_mic_sidecar_path")]
    pub mic_sidecar_path: String,
    #[serde(default)]
    pub log: LogConfig,
    #[serde(default)]
...
impl Default for Config {
    fn default() -> Self {
        Self {
            ...
            mic_sidecar_path: default_mic_sidecar_path(),
...
fn default_streamer_path() -> String {
    if cfg!(windows) {
        "./streamer.exe".to_string()
    } else {
        "./streamer".to_string()
    }
}

fn default_mic_sidecar_path() -> String {
    if cfg!(windows) {
        "./mic_sidecar.exe".to_string()
    } else {
        "./mic_sidecar".to_string()
    }
}
